Sub RefreshLegacyView()
    ' Dynamic Legacy View Rebuilder - No External Dependencies
    ...
    If lastRow > 3 Then
        ' More specific range clearing to avoid range errors
        If lastRow > 1048576 Then lastRow = 1048576 ' Excel row limit
        Dim clearRange As String
        clearRange = "A4:ZZ" & lastRow
        legacyWs.Range(clearRange).ClearContents
        legacyWs.Range(clearRange).ClearFormats
    End If
    On Error GoTo ErrorHandler
